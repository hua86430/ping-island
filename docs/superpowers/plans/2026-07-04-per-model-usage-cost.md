...
- [ ] 寫失敗測試 `PingIslandTests/AgentUsageModelPricingTests.swift`（覆蓋 spec「model id 正規化規則」每一條與 fallback）：
...
- [ ] 跑測試確認編譯失敗（型別不存在即為預期失敗）：`DEVELOPER_DIR=/Applications/Xcode.app/Contents/Developer xcodebuild -project PingIsland.xcodeproj -scheme PingIsland -configuration Debug CODE_SIGNING_ALLOWED=NO test -only-testing:PingIslandTests/AgentUsageModelPricingTests`
- [ ] 建立 `PingIsland/Services/Usage/AgentUsageModelPricing.swift`，最小實作：
...
- [ ] 跑測試確認通過：`DEVELOPER_DIR=/Applications/Xcode.app/Contents/Developer xcodebuild -project PingIsland.xcodeproj -scheme PingIsland -configuration Debug CODE_SIGNING_ALLOWED=NO test -only-testing:PingIslandTests/AgentUsageModelPricingTests`
- [ ] Commit：`feat: add AgentUsageModelPricing registry with per-model official rates`
...
- [ ] 在 `AgentUsageAnalyticsTests.swift` 加失敗測試：
...
- [ ] 跑測試確認失敗：`DEVELOPER_DIR=/Applications/Xcode.app/Contents/Developer xcodebuild -project PingIsland.xcodeproj -scheme PingIsland -configuration Debug CODE_SIGNING_ALLOWED=NO test -only-testing:PingIslandTests/AgentUsageAnalyticsTests`
- [ ] 修改 `AgentUsageDailyBucket`：加欄位、自訂 `init(from:)`（舊桶缺 key 解為空 map；有自訂 `CodingKeys` 時 `encode(to:)` 仍可用合成版，不必手寫）、加 `recordTokens(perModel:)`：
...
- [ ] 跑測試確認通過（同上 `-only-testing:PingIslandTests/AgentUsageAnalyticsTests`）
- [ ] Commit：`feat: track per-model token totals in AgentUsageDailyBucket`
...
- [ ] 加失敗測試（`ClaudeTranscriptUsageLoaderTests.swift`）：
...
- [ ] 跑測試確認失敗：`DEVELOPER_DIR=/Applications/Xcode.app/Contents/Developer xcodebuild -project PingIsland.xcodeproj -scheme PingIsland -configuration Debug CODE_SIGNING_ALLOWED=NO test -only-testing:PingIslandTests/ClaudeTranscriptUsageLoaderTests`
- [ ] 最小實作：snapshot 加欄位、`load` 逐行分桶、加 `modelIdentifier(from:)`：
...
- [ ] 跑測試確認通過（同 class filter；`ClaudeTranscriptUsageLoaderTests` 既有 5 個測試 —— 4 個 loader + 1 個 record —— 也必須綠，混 model 之外的 transcript 落在 `unknown` 鍵）
- [ ] Commit：`feat: bucket Claude transcript usage per message model`
...
- [ ] 在 `CodexUsageLoaderTests.swift` 加失敗測試：
...
- [ ] 跑兩個 class 確認失敗：`DEVELOPER_DIR=/Applications/Xcode.app/Contents/Developer xcodebuild -project PingIsland.xcodeproj -scheme PingIsland -configuration Debug CODE_SIGNING_ALLOWED=NO test -only-testing:PingIslandTests/AgentUsageAnalyticsTests -only-testing:PingIslandTests/CodexUsageLoaderTests`
- [ ] 改 `CodexTokenUsage`（`AgentUsageAnalytics.swift:367-376`）：
...
- [ ] 跑兩個 class 確認通過（既有 `testLoadParsesLastTokenCountRateLimits` 的 `tokenUsage` 等值斷言因 `cachedInputTokens` 預設 0 仍成立）
- [ ] Commit：`feat: split Codex cached input tokens and capture rollout model`
...
- [ ] 在 `AgentUsageAnalyticsTests.swift` 加失敗測試（含 spec「測試」節的 recordTokenUsage B 全部情境）：
...
- [ ] 跑測試確認失敗（編譯錯誤即為預期）：`DEVELOPER_DIR=/Applications/Xcode.app/Contents/Developer xcodebuild -project PingIsland.xcodeproj -scheme PingIsland -configuration Debug CODE_SIGNING_ALLOWED=NO test -only-testing:PingIslandTests/AgentUsageAnalyticsTests`
- [ ] 改 `AgentUsageTokenSourceBaseline`：
...
- [ ] 改 `AgentUsageDocument` 兩處遷移迴圈（433-438 與 476-481 行），一律建空 map：
...
- [ ] 改 `recordCodexUsageSnapshot`（615-636 行）：一次到位帶入 model 鍵穩定規則（nil 時沿用既有 baseline 唯一鍵，連前次都沒有才 `unknown`），不留翻轉窗（spec fable5 3b 標 BLOCKER，不可分兩 commit）。Task 6 只補回歸測試守這段：
...
- [ ] 刪除 `AgentUsageDailyBucket.recordTokens(_ totals: AgentUsageTokenTotals)`（此改動後無呼叫者）
- [ ] 改 `SessionStore.recordClaudeFamilyTranscriptUsageIfAvailable`（2819-2834 行）：`totals: snapshot.tokenTotals` 改為 `totalsByModel: snapshot.tokenTotalsByModel`，其餘參數（含 `recordInitialSnapshot: false`、sourceKey）不動
- [ ] 更新 `ClaudeTranscriptUsageLoaderTests.testRecordTranscriptUsageDoesNotDoubleCountRepeatedReads`：三處 `totals: firstSnapshot.tokenTotals` / `totals: secondSnapshot.tokenTotals` 改為 `totalsByModel: firstSnapshot.tokenTotalsByModel` / `totalsByModel: secondSnapshot.tokenTotalsByModel`（斷言不變；該測試未帶 `recordInitialSnapshot:` 走預設 true，首錄即計入，行為與改動前相同）
- [ ] 跑整包確認通過（此 task 動到共用路徑）：`DEVELOPER_DIR=/Applications/Xcode.app/Contents/Developer xcodebuild -project PingIsland.xcodeproj -scheme PingIsland -configuration Debug CODE_SIGNING_ALLOWED=NO test -only-testing:PingIslandTests`
- [ ] Commit：`feat: compute per-model token deltas against per-source baselines`
...
- [ ] 跑 class 確認通過（回歸鎖：Task 5 已實作，應一次就過）：`DEVELOPER_DIR=/Applications/Xcode.app/Contents/Developer xcodebuild -project PingIsland.xcodeproj -scheme PingIsland -configuration Debug CODE_SIGNING_ALLOWED=NO test -only-testing:PingIslandTests/AgentUsageAnalyticsTests`
- [ ] Commit：`test: lock Codex per-model baseline key stability when model scan misses`
...
- [ ] 跑測試確認失敗（同 class filter）
- [ ] 實作。新 struct 照 Interfaces 區塊原樣加入。`AgentUsageDashboardSnapshot` 加兩個 `let` 欄位；`empty` 補 `perModelBreakdown: []`、`perModelDailySpend: []`。加殘量計價 helper（放 `AgentUsageDashboardSnapshot` extension 區）：
...
- [ ] `en.lproj/Localizable.strings` 在 `"Codex / Claude Code 均价"` 那行後加入：
...
- [ ] `zh-Hant.lproj/Localizable.strings` 同位置加入（值必為繁體，逐字檢查用字）：
...
- [ ] 驗證兩檔語法：`plutil -lint PingIsland/Resources/en.lproj/Localizable.strings && plutil -lint PingIsland/Resources/zh-Hant.lproj/Localizable.strings`
- [ ] Commit：`feat: add per-model usage i18n strings`
...
- [ ] 建立 `PingIsland/UI/Views/Settings/Categories/AgentUsagePerModelViews.swift`：
...
- [ ] 改 `AnalyticsSettingsView.swift`：`body` 的 `spendCard` 與 `activityMapCard` 之間插入 `perModelCard`，並新增：
...
- [ ] 改 `AgentUsageRows.swift`：`AgentUsageSpendPanel` 加 `let usesPerModelPricing: Bool` 並把它傳給 footer（`AgentUsageSpendFooter(summary: summary, usesPerModelPricing: usesPerModelPricing)`）；`AgentUsageSpendFooter` 加同名欄位，`pricingLabel` 改為：
...
- [ ] 建置確認綠：`DEVELOPER_DIR=/Applications/Xcode.app/Contents/Developer xcodebuild -project PingIsland.xcodeproj -scheme PingIsland -configuration Debug CODE_SIGNING_ALLOWED=NO build`（若接管線過濾輸出，驗 `${PIPESTATUS[0]}` / zsh `${pipestatus[1]}`）
- [ ] 跑整包單元測試確認綠：`DEVELOPER_DIR=/Applications/Xcode.app/Contents/Developer xcodebuild -project PingIsland.xcodeproj -scheme PingIsland -configuration Debug CODE_SIGNING_ALLOWED=NO test -only-testing:PingIslandTests`
- [ ] Commit：`feat: add per-model usage and cost card to analytics settings`
...
- [ ] 全量回歸：`DEVELOPER_DIR=/Applications/Xcode.app/Contents/Developer xcodebuild -project PingIsland.xcodeproj -scheme PingIsland -configuration Debug CODE_SIGNING_ALLOWED=NO test -only-testing:PingIslandTests`，再跑一次 Debug build，兩者 exit code 都必須為 0
- [ ] 對照 spec 成功條件逐項檢核：新卡出現（清單依花費降序、多線圖含「其他」）、Opus 4.8 以 5/25 計、Haiku 1/5、gpt-5.5 5/30、未知走 blend、Codex cached 0.1x、headline 逐 model 加總、舊資料不歸零、英文介面無簡體洩漏（切 en 語系目視統計頁）
- [ ] `AGENTS.md` 的 `PingIsland/Services/Usage` 兩處描述補「per-model official pricing / per-model usage breakdown」字樣，讓路由層反映 `AgentUsageModelPricing.swift` 與逐模型統計入口
- [ ] spec 檔狀態行「待實作」改為「已實作（2026-07-04 plan 完成）」；若實作過程中任何決策偏離 spec，先改 spec 再收尾
- [ ] Commit：`docs: sync AGENTS.md and spec status for per-model usage cost`
