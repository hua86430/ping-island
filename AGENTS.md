...
- Whenever code or architecture changes, `ARCHITECTURE.md` changes with it — a code change is NOT done until the doc matches. This is per code change, not scoped to a PR or commit boundary: touched file → update its section text and diagrams; new file → add a matrix row and fold it into a section; deleted file → remove its matrix row; changed cross-subsystem flow → update the "跨切面流程索引" plus the affected section diagrams.
- This repo often develops docs-first with superpowers (`brainstorming` → `writing-plans`; specs/plans live under `docs/superpowers/`). When a spec or plan describes a code change, that plan MUST include updating `ARCHITECTURE.md` and its coverage matrix as an explicit task/step — the doc update is part of the work item, so it ships with the implementation instead of drifting behind it.
...
- Always-on menu bar status item (Settings escape hatch for the accessory app): `PingIsland/App/StatusBarController.swift` (menu layout in the pure `StatusBarMenuBuilder`, held by `AppDelegate`); switchable icon in `PingIsland/App/MenuBarIconStyle.swift` (template imagesets, SVG sources in `design/menubar-icons/`), picked in Settings → Display
...
- Global shortcuts and shortcut persistence: `PingIsland/Services/Shared/GlobalShortcutManager.swift`, `PingIsland/Utilities/GlobalShortcut.swift`, `PingIsland/Core/Settings.swift`, `PingIsland/UI/Views/Settings/Categories/ShortcutsSettingsView.swift`
...
- If you add a Claude-compatible hook client, start in `PingIsland/Models/ClientProfile.swift` and wire any truly client-specific behavior from there before adding new ad-hoc switches elsewhere.
  - When `AppSettingsDefaultKeys.terminalHandlesAskUserQuestion` is set, `HookInstaller.effectiveEvents` rewrites the Claude Code profile's (`id == "claude-hooks"`) `PreToolUse` and `PermissionRequest` matchers from `"*"` to `HookInstaller.askUserQuestionExclusionMatcher` (a negative-lookahead regex excluding `AskUserQuestion`/`AskFollowupQuestion`), so Claude renders its native terminal picker and the island shows nothing for that question; the Settings toggle reinstalls Claude hooks and it takes effect on the next Claude session. `PostToolUse` stays `"*"`; the exclusion is scoped to `claude-hooks` only.
...
- If you change session lifecycle or transitions, start in `SessionStore`. Avoid ad-hoc state mutation elsewhere.
  ...
  - When `notificationFeedMode` is on, the opened Island's `.sessionList` and `.hoverDashboard` routes render `NotificationFeedView` instead of the session list/dashboard: unread-only (`SessionState.hasUnread` = `lastNotifiableActivityAt > lastSeenAt`; only assistant-reply completion bumps `lastNotifiableActivityAt` — the hook-family `Stop` event and the Codex processing→idle transition — never SessionStart/typing/tool churn; in-memory, so the feed starts empty on launch), exempt from the 30-minute idle hide via `isHiddenFromPrimaryUIOnlyByIdle`, tap = focus terminal + `SessionStore.markSessionSeen`, Clear All = `markAllSessionsSeen`, and the collapsed island shows only the unread-count badge (session count hidden). Feed mode also decouples WHEN the island auto-opens: a bare new-session/prompt-ready pending never opens it; actionable attention (`needsPromptNotification`) opens and stays; the completion card keeps its own 5-second flow; any other notification-opened panel left on the feed route self-closes after 5 seconds with completion-style hover pause (`NotchAutoOpenPolicy` holds the pure WHEN decision functions; the feed-banner timer lives beside the completion dismissal machinery in `NotchView`). Trace `SessionState`, `SessionStore`, `SessionMonitor.filteredVisibleSessions`, `NotchAutoOpenPolicy`, `NotchView`, `IslandOpenedContentView`, and `NotificationFeedView` together; toggle off must stay behavior-identical to today (both WHAT renders and WHEN it opens).
...
- If you change notch sizing, opening behavior, or visibility, inspect both `NotchViewModel` and `NotchView`.
  ...
  - Docked-notch hover open/close works at all energy levels (including idle), NOT via the energy-gated global `.mouseMoved` monitor: `NotchHoverSensorWindow` (a nonactivating near-transparent panel over the trigger rect with an `.activeAlways` NSTrackingArea) fires hover-open; a bounded `NotchViewModel.hoverCloseTimer` (runs only while hover-opened) closes once the cursor leaves `openedHoverRegionRect`. Sensor rect = `NotchViewModel.hoverSensorRect` (nil when detached / suppress-hidden, reveal rect in fullscreen edge-reveal), refreshed from `updateWindowPresentation` via `$status/$openReason/$geometry/$closedWidth/...` sinks. Click-open and drag-detach are unchanged — they ride the local `NSEvent` monitor into `handleMouseDown`. Hover-open uses reason `.hover`, excluded from `NSApp.activate`/`makeKey` so it never steals terminal focus. A tracking-area close was tried and abandoned (the opened full-size window occludes the sensor/overlay). Trace `NotchHoverSensorFrame`, `NotchViewModel.hoverSensorRect`/`hoverCloseTimer`, and `NotchWindowController.updateWindowPresentation` together when changing hover.
  - The docked notch window frame is status-scaled so the transparent panel does not span half the screen while idle (screen-capture window pickers used to highlight the full 750pt canvas): `.closed` shrinks to a full-width top strip (`closedHeight` + `NotchWindowController.closedFrameSlack`), `.opened`/`.popping` grow to the full 750pt canvas. Single chokepoint is `NotchWindowController.updateWindowPresentation` (grow immediately before showing; shrink after `closedFrameShrinkDelay`, past the collapse animation, re-checking `.closed` so a re-open cancels it), sharing `targetWindowFrame(status:screenFrame:closedHeight:)` with `moveToScreen`. `NotchViewController.panelHitRect` reads the live window height, not a hardcoded 750 — keep hit-testing and frame scaling aligned, and re-assert `hasShadow = false` after every `setFrame`.
...
- If you change the persisted surface mode or first-run onboarding, trace through `AppDelegate`, `WindowManager`, `IslandPresentationCoordinator`, `SettingsWindowController`, `SettingsWindowView`, `StatusBarController`, and `Settings.swift` together so launch-time routing and in-app switching stay aligned.
- If you change the menu bar status item or its menu (Settings / 展示模式 submenu / 檢查更新 / 離開), trace through `PingIsland/App/StatusBarController.swift` (keep menu layout in the pure `StatusBarMenuBuilder` with matching `StatusBarMenuBuilderTests`), `AppDelegate` (owns it), and `Settings.swift` (surface-mode writes) plus the `IslandPresentationCoordinator` `$surfaceMode` sink. The menu must only write `AppSettings.surfaceMode`, never mutate presentation directly. New menu strings follow the Simplified-key / Traditional-value convention and must pass `check-simplified-chinese.swift`.
- If you change the switchable menu bar icon, trace through `PingIsland/App/MenuBarIconStyle.swift` (enum + `Assets.xcassets` template imagesets, SVG sources in `design/menubar-icons/`), `Settings.swift` (`menuBarIconStyle` persistence, four-part pattern), `StatusBarController` (`$menuBarIconStyle` sink re-renders the button), and `MenuBarIconStylePicker` in `PingIsland/UI/Views/Settings/Categories/DisplaySettingsView.swift`, keeping `MenuBarIconStyleTests` and the style name localization keys in sync. Adding an icon = new imageset + enum case + title key (Simplified key / Traditional + English value).
- If you change the Settings window, the shell lives in `PingIsland/UI/Views/Settings/` (`SettingsRootView` = NavigationSplitView shell, `SettingsSidebarView`, `SettingsDetailRouter`, `SettingsPanelViewModel`, `Components/`, `Categories/` one file per category); `SettingsWindowView.swift` is only the thin entry wrapper shared by the Settings scene and `SettingsWindowController`. The window uses native titled `.fullSizeContentView` chrome: content is positioned below the traffic lights by the native titlebar safe area (no manual top padding), and `hostingController.sizingOptions = []` keeps the window at its `SettingsWindowController` content size instead of growing to the SwiftUI ideal height. Built against the macOS 26 SDK the sidebar picks up native Liquid Glass; keep glass on the navigation layer only (sidebar), not on detail content. Do not reintroduce `safeAreaRegions = []`, `ignoresSafeArea(.top)`, fixed `.frame(min/ideal/max)` window sizing, manual sidebar top padding, custom window controls, drag handles, or the removed `.popover` presentation mode.
- If you change global shortcuts, shortcut persistence, or shortcut hints, trace through `PingIsland/Services/Shared/GlobalShortcutManager.swift`, `PingIsland/Utilities/GlobalShortcut.swift`, `PingIsland/Core/Settings.swift`, `PingIsland/UI/Views/Settings/Categories/ShortcutsSettingsView.swift`, `PingIsland/UI/Components/GlobalShortcutHintView.swift`, and the relevant notch/chat/session-list views together so registration, customization, and visible hints stay aligned.
