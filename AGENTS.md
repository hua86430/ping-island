...
- If you change session lifecycle or transitions, start in `SessionStore`. Avoid ad-hoc state mutation elsewhere.
  ...
  - When `notificationFeedMode` is on, the opened Island's `.sessionList` route renders `NotificationFeedView` instead of `SessionListView`: unread-only (`SessionState.hasUnread` = `lastActivity > lastSeenAt`, in-memory, defaults to seen at creation so the feed starts empty on launch), exempt from the 30-minute idle hide, tap = focus terminal + `SessionStore.markSessionSeen`, Clear All = `markAllSessionsSeen`, and the collapsed island shows an unread-count badge. Trace `SessionState`, `SessionStore`, `SessionMonitor.filteredVisibleSessions`, `IslandOpenedContentView`, and `NotificationFeedView` together; toggle off must stay behavior-identical to today.
