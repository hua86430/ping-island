...
- **Dwell / throttle.** The cursor must rest on the new screen for a short interval before migration (default 200 ms). This suppresses migration when the cursor only passes through a screen. Value is a single tunable constant.
